...
const Toast = React.forwardRef(({ className, ...props }, ref) => <ToastPrimitives.Root ref={ref} className={cn('group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-6 pr-8 shadow-lg', className)} {...props} />);
...
const ToastClose = React.forwardRef(({ className, ...props }, ref) => <ToastPrimitives.Close ref={ref} className={cn('absolute right-2 top-2 rounded-md p-1 text-foreground/50 opacity-70', className)} {...props}><X className="h-4 w-4" /></ToastPrimitives.Close>);
